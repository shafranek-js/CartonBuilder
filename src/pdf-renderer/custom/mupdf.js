...
export class PDFPage extends Page {
    ...
    toPixmapWithOverprint(matrix, colorspace, alpha = false, showExtras = true, usage = "Print", box = "CropBox", overprintMode = 0) {
        checkMatrix(matrix);
        checkType(colorspace, ColorSpace);
        let box_ix = ENUM(box, Page.BOXES);
        let result;
        if (showExtras)
            result = libmupdf._wasm_pdf_new_pixmap_from_page_with_usage_and_overprint(this.pointer, MATRIX(matrix), colorspace.pointer, alpha, STRING(usage), box_ix, overprintMode);
        else
            result = libmupdf._wasm_pdf_new_pixmap_from_page_contents_with_usage_and_overprint(this.pointer, MATRIX(matrix), colorspace.pointer, alpha, STRING(usage), box_ix, overprintMode);
        return new Pixmap(result);
    }
    toPixmapWithOverprintAndBehaviors(matrix, colorspace, alpha = false, usage = "Print", box = "CropBox", overprintMode = 0, behaviors = null) {
        checkMatrix(matrix);
        checkType(colorspace, ColorSpace);
        let box_ix = ENUM(box, Page.BOXES);
    ...
    separationNames() {
        ...
        let names = [];
        try {
            libmupdf._wasm_pdf_page_separation_names(this.pointer, buf, buflen);
            let bytes = libmupdf.HEAPU8.subarray(buf, buf + buflen);
            let start = 0;
            for (let i = 0; i < bytes.length && names.length < count; i++) {
                if (bytes[i] === 0) {
                    if (i > start)
                        names.push(decoder.decode(bytes.subarray(start, i)));
                    start = i + 1;
                }
            }
        }
    ...
    separationCoverage(overprintMode = 0) {
        let count = this.separationCount();
        let total = 4 + count;
        let buf = libmupdf._wasm_malloc(total * 4);
        try {
            libmupdf._wasm_pdf_page_separation_coverage(this.pointer, buf, total, overprintMode);
            let out = [];
            for (let i = 0; i < total; i++)
                out.push(libmupdf.HEAP32[(buf >> 2) + i]);
            return out;
        }
        finally {
            libmupdf._wasm_free(buf);
