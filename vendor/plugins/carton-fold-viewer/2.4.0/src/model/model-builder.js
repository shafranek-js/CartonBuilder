...
export function buildFoldableFromSemanticSvg(svgText, name = 'carton.svg') {
  const parsed = parseSemanticCartonSvg(svgText), model = new THREE.Group();
  ...
  for (const s of parsed.specs) {
    ...
    const mesh = new THREE.Mesh(
      makePanelGeometry(s.polygon, parsed.dimensions.thickness, s),
      makeTechMaterials(s)
    );
    mesh.name = s.nodeName + '__mesh';
    mesh.userData = { panel_id: s.id, semantic_role: s.semanticRole, kind: s.kind, layer_class: s.layerClass };
  ...
  for (const f of Object.values(parsed.folds)) {
    ...
    const mesh = new THREE.Mesh(
      makeFiniteCreaseGeometry(f, parsed, parentOrigin, parsed.creaseProfile),
